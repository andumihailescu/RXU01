...
namespace
{
    ...
    std::atomic<uint32_t>
        g_dropped_receive_packets{0};

    std::atomic<uint32_t>
        g_dropped_send_results{0};
    ...
    {
        constexpr uint8_t
            ZERO_MAC[esp_now_driver::MAC_ADDRESS_SIZE]{};
        ...
        int data_length)
    {
        if (!g_initialized.load(
            ...
            data_length >
                static_cast<int>(
                    esp_now_driver::MAX_PAYLOAD_SIZE))
        {
            return;
        }
...
namespace esp_now_driver
{
    esp_err_t init(const Config &config)
    {
        ...
        result = esp_now_register_recv_cb(
            onDataReceived);
        ...
        result = esp_now_register_send_cb(
            onDataSent);
